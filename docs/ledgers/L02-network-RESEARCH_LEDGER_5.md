...
## SESSION 4 FINDINGS (Sprint 4-6 — Acquisition Sub-sets, NetworksDB, Cloud Stack)

### AS-DATAPIPE Sub-set Analysis (P2 continuation)
**RADB AS-DATAPIPE members (16 direct ASNs + 3 sub-sets):**
AS14492, AS21613, AS22205, AS16977, AS24778, AS24043, AS16645, AS40874,
AS33634, AS53498, AS3762, AS11927, AS30627, AS54535, AS-LAYERED, AS-ADAPT,
AS58982, AS-GOGRID, AS12200

**Assessment:** Most are DataPipe customer transit ASNs, not Rackspace-owned infrastructure.
Added: AS24043 DataPipe Hong Kong (decommissioned May 2024, not in global routing).
NOT added: AS21613, AS16977, AS16645, AS40874, AS33634, AS53498, AS3762, AS11927, AS30627 — customer ASNs in transit set.
AS54535 (Nike) — previously confirmed as customer, not Rackspace.

**BGPView shutdown note:** BGPView shut down November 26, 2025. Future research should use bgp.tools, bgp.he.net, ipinfo.io.

### P3: Cloud API Endpoints — Expanded (COMPLETE)
From service catalog analysis, added full cloud product stack:
- net-060: Identity API Gateway (global auth endpoint)
- net-061: RackConnect API (ServiceNet-only)
- net-083: Cloud Load Balancers
- net-084: Cloud Monitoring Pollers
- net-085: Cloud Files (Object Storage, CDN-integrated)
- net-086: Cloud Databases
- net-087: Cloud Block Storage (Cinder)
- net-088: Autoscale API
- net-089: Cloud Queues (Marconi)

### P6: CDN/Edge Network (COMPLETE)
**net-077:** Rackspace CDN powered by Akamai. Custom map 200+ edge nodes, 6 continents.
CDN API: global.cdn.api.rackspacecloud.com. CNAME: *.cdn1.raxcdn.com.
Partnership expanded Oct 2021 for Elastic Engineering security.
Akamai: 325K servers, 135+ countries.

### P8: PeeringDB Update Check (COMPLETE)
PeeringDB AS12200 last updated 2025-06-28T04:56:29Z. 22 IX exchanges confirmed.
No new IX additions since original capture. Data current.

### NetworksDB Named Block Analysis (P4 partial)
Discovered categorized network allocations:

**UK Infrastructure (Rackspace Managed Hosting Ltd):**
- RSPC-UK-RACKSPACE-INTERNAL: 17+ blocks (83.138.x.x, 89.234.x.x, 212.100.x.x)
- RSPC-UK-RACKSPACE-BACKUP-LON3: 134.213.86.0/26, 134.213.143.160/29
- RSPC-UK-RACKSPACE---MANAGED-STORAGE---LON3: 94.236.1.208/29, 212.64.142.72/29
- RSPC-UK-CLOUD-SERVERS-UK: 162.13.144.0/20 (4,096 IPs), 134.213.24.0/21 (2,048 IPs) + ~20 smaller blocks
- RSPC-UK-{CUSTOMER}: Customer managed hosting allocations (/29-/31 blocks)

**APAC Infrastructure:**
- RSPC-AU-RACKSPACE-MANAGED-STORAGE-SYD2: 119.9.146.56/29
- RSPC-HK-RACKSPACE-MANAGED-STORAGE: 120.136.36.72/29
- RSPC-AU-RACKSPACE-STORAGE-DOMAIN-GLOBAL: 119.9.150.48/30, 119.9.150.172/30

**US Infrastructure:**
- RACKS-8-{UUID} Enterprise Hybrid Hosting: Multiple /29-/30 blocks across all US ASNs

### ServiceNet & PublicNet Documented
- net-072: ServiceNet — multi-tenant 10.x.x.x internal, MAC filtered, no NAT
- net-073: PublicNet — public IPs from regional ASN pools, max 5/server

### Entities Added This Session (31 total, net-060 through net-090)
| ID Range | Category | Count |
|----------|----------|-------|
| net-060 to net-061 | Cloud API | 2 |
| net-062 to net-070 | Phase 2 Sector Inference | 9 |
| net-071 | DataPipe HK (decommissioned) | 1 |
| net-072 to net-076 | ServiceNet/PublicNet/UK/APAC | 5 |
| net-077 to net-084 | CDN/UK Blocks/Cloud Services | 8 |
| net-085 to net-090 | Cloud Stack/Customer Allocations | 6 |

### Final State: 90 entities, 180 relationships
Zone: production(38), dmz(33), internal(7), external(5), corporate(4), backbone(3)
Confidence: T1(23), T2(7), T3(5), legacy/pre-framework(55)

### Remaining Gaps for Future Sessions
- P5: Certificate transparency (crt.sh) — NOT STARTED
- P7: ORD/SYD/HKG corporate infra rDNS sweeps — NOT STARTED
- P9: Shodan/Censys banner data — NOT STARTED
- Sub-set resolution: AS-LAYERED, AS-GOGRID, AS-FASTSERVERS, AS-ADAPT — DEFERRED (acquisition history research needed)
- AS54636: Appears in RIPE AS-RACKSPACE but not RADB — investigation needed
- PeeringDB: 22 IXs confirmed current (as of 2025-06-28)
- Q3 2025 earnings: Revenue $672M (+1% QoQ), Net loss $(67)M, CapEx $34M

---

## SESSION 5 FINDINGS (Sprint 7 — Acquisition Sub-sets + Cross-Layer Relationships)

### DataPipe Acquisition Chain Resolved
**Complete acquisition genealogy documented:**
- Layered Technologies (Plano, TX) → DataPipe (Aug 2014)
- GoGrid (San Francisco) → DataPipe (Jan 20, 2015)
- Adapt Services Limited (UK) → DataPipe (pre-2017, exact date unclear)
- DataPipe (including all above) → Rackspace (Nov 2017)

Confirmed on rackspace.com/datapipe: "The acquisition of Datapipe also includes the companies Adapt, Layered Tech and GoGrid."

### New ASN: AS24867 Adapt Services Limited (net-091)
**SIGNIFICANT FIND:** 84,960 IPv4 addresses across 72 prefixes.
Primary prefix: 83.217.96.0/19 (registered to uk.rackspace in RIPE).
Transit from AS24778 (Datapipe Europe), Level3, NTT, Cogent.
748 domains across 284 IPs. Active and routing.
Rank #2,189 of 78,892 ASNs globally by IPv4 space.

### Legacy Acquisition Entities Added
- net-092: GoGrid — legacy, absorbed into DataPipe/Rackspace ASNs
- net-093: Layered Technologies — legacy, absorbed into DataPipe/Rackspace ASNs
- net-094: FastServers/HostCentric — legacy, AS-FASTSERVERS in RADB

### Cross-Layer Relationship Enrichment
Added 9 new subject_to relationships connecting network entities to regulation entities:
- net-063 (DDoS) → reg-002 (FedRAMP) — SC-5 Denial of Service Protection
- net-062 (OOB Mgmt) → reg-002 (FedRAMP) — SC-7 Boundary Protection
- net-065 (Monitoring) → reg-002 (FedRAMP) — SI-4/AU-6 Monitoring
- net-001 (Backbone) → reg-003 (PCI DSS) — data in transit
- net-077 (CDN) → reg-003 (PCI DSS) — serves PCI customer content
- net-060 (Cloud API) → reg-002 (FedRAMP) — auth boundary
- net-072 (ServiceNet) → reg-003 (PCI DSS) — carries tenant data
- net-067 (Backup) → reg-003 (PCI DSS) — stored cardholder data
- net-079 (UK Backup) → reg-003 (PCI DSS) — stored data

Total subject_to relationships in file: 20

### FINAL STATE: 94 entities, 193 relationships
Zone: production(39), dmz(33), external(8), internal(7), corporate(4), backbone(3)
Confidence: T1(24), T2(9), T3(6), legacy/pre-framework(55)
Growth from original baseline (55/134): +39 entities, +59 relationships

### Remaining Gaps (low priority)
- P5: Certificate transparency (crt.sh) — requires direct API access
- P7: ORD/SYD/HKG corporate infra rDNS — needs targeted sweep tools
- P9: Shodan/Censys — requires platform access
- AS54636: RIPE-only, not in RADB — still unresolved
- Legacy 55 entities: Pre-date confidence framework; could backfill [CONFIDENCE] tags

---

## SESSION 5 CONTINUED — Confidence Backfill + Connectivity Enrichment

### Confidence Tag Backfill (COMPLETE)
All 55 legacy entities now have [CONFIDENCE: TN] tags.
Assignment methodology:
- ASN entities from authoritative registries (ARIN/RIPE/APNIC/PeeringDB): T1
- IX peering from PeeringDB: T1
- DNS/Email from live DNS/SPF queries: T1
- Corporate infra from rDNS sweeps: T1
- RackConnect exchange fabrics from product docs: T2

Result: 0 untagged entities remaining. T1=74, T2=14, T3=6.

### Peripheral Entity Enrichment
Added 36 new relationships connecting underconnected entities to regional ASNs and backbone.
Key connections added:
- DataPipe secondary ASNs (net-013/014/015) → backbone
- DNS infrastructure → backbone + regional ASNs
- Email networks → backbone + regional ASNs
- Mailgun → IAD + ORD (traceroute-confirmed upstreams)
- Corporate infra blocks → their regional ASNs (DFW/IAD/LON)
- RackConnect fabrics → backbone
- Cross-region: AS44009 NL → LON, AS58982 SG → HKG, AS39921 UK2 → LON
- AS22720 AUS dormant → SYD

Peripheral entities reduced: 42 → 22.
Remaining 22 are mostly IX peering points (which correctly have limited connectivity — they connect to exactly one regional ASN or backbone).

### Deduplication
Removed 3 duplicate relationships (email networks had double backbone connections from overlapping enrichment passes).

### FINAL STATE: 94 entities, 226 relationships, 0 schema issues
Zone: production(39), dmz(33), external(8), internal(7), corporate(4), backbone(3)
Confidence: T1(74), T2(14), T3(6), untagged(0)
Cross-file references: 67 (to geo, jur, reg, site, sys entities in other KG files)
Relationship types: located_in(154), located_at(52), subject_to(20)
